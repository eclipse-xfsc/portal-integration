...
import { ArticleCategory } from '../components/article-news/utils/ArticleNews-model';
...
const useArticleService = <T>(category: ArticleCategory, handleError?: (error: Error) => void) => {
  const [result, { fetchData, sendData }] = useApi<T>(handleError);

  const load = useCallback(() => {
    fetchData({ ...config, url: `${config.url}filter?category=${category}` });
  }, [category, fetchData]);
